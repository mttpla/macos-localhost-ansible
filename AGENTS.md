...
`libreoffice` was dropped from this list on 2026-09-09: its Caskroom receipt was deleted deliberately (repeated upgrade failures — the cask kept finding a stale app already at the target path and refusing to purge it, requiring interactive sudo this machine can't supply non-interactively). Homebrew no longer tracks it or updates it. The app itself is still sitting in `/Applications` unremoved — same `root:admin` permissions problem as the other nine, so removing it for real still needs an IT request.
